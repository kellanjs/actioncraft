...
describe("utils - unwrap and throwable", () => {
  ...
  describe("isActioncraftError type guard", () => {
    ...
    it("should validate ActioncraftError structure", async () => {
      ...
      const validMessageAction = actioncraft()
        .errors({
      ...
      const undefinedMessageAction = actioncraft()
        .errors({
          testError: () => ({ type: "TEST_ERROR" as const }),
        })
        .handler(async ({ errors }) => errors.testError())
    ...
    it("should support verified error checking with action IDs", async () => {
      ...
      const { isActioncraftError } =
        await import("../../../src/classes/error.js");
  ...
  describe("real-world integration patterns", () => {
    it("should work with React Query-style error handling", async () => {
      const updateUserAction = actioncraft()
        .schemas({
          inputSchema: z.object({
            id: z.string(),
            name: z.string().min(2),
    ...
    it("should work with useQuery patterns", async () => {
      const getWorkspaces = actioncraft()
        .schemas({
...
describe("isStandardSchema utility", () => {
  it("should return true for valid Standard Schema v1 objects", () => {
    const validSchema = {
      "~standard": {
        version: 1,
        vendor: "test",
  ...
  it("should return false for objects with invalid ~standard structure", () => {
    // ~standard is not an object
  ...
  it("should return true for Zod schemas (real-world Standard Schema)", () => {
    // Zod implements Standard Schema v1
    expect(isStandardSchema(stringSchema)).toBe(true);
    expect(isStandardSchema(numberSchema)).toBe(true);
